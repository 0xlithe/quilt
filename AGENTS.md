...
- Like success visual feedback uses a custom heart SVG overlay animation (spring, sparkle, subtle article pulse), not a full article fade-to-black. Unlike retains article fade, height collapse, and DOM removal.
...
- `chrome.storage.local` holds followed/liked ID sets, daily session counters, debug enablement, sidebar preference, cooldown state, first-seen timestamp, saved searches, search categories with colors, search template init flag, and sidepanel tab state.
- All four task types (Follow, Unfollow, Like, Unlike) use direct API requests via the MAIN-world bridge. Follow/Unfollow fire `friendships/create.json`/`destroy.json`; Like/Unlike fire GraphQL `FavoriteTweet`/`UnfavoriteTweet`. Query IDs are captured dynamically with hardcoded fallbacks. Unfollow targets all visible users on the following page (not restricted to Quilt-followed accounts) so users can mass-unfollow freely.
- `pageWorldFollowTap.js` runs in the MAIN world, patches `fetch`/`XHR` to intercept friendship and GraphQL requests, and handles bridge messages for direct API calls. All `postMessage` listeners validate `ev.origin` against x.com/twitter.com, and API base URLs derive from `location.origin`.
- `TaskRunner.cancel()` force-clears `_running` and uses an epoch counter so stale async loops cannot clobber a new task's state. The main loop checks epoch equality to exit silently when superseded. `emitStatus` payloads include `taskType`, `startedAt`, `completed`, and `maxActions`.
- The Chrome side panel (`sidepanel.html`/`sidepanel.js`) is the primary UI with **Tasks** and **Search** tabs. The Search tab has Builder, Saved Searches, and Categories sub-views ported from X Search Pro (`queryBuilder.js`, `searchStorage.js`, `searchTemplates.js`, `searchApply.js`). Extension options page hosts sidebar toggle and debug logging.
- Bridge messaging between isolated and MAIN worlds uses a crypto nonce stored on `document.documentElement` plus `ev.origin` validation. The nonce is visible to same-page scripts but combined with origin checks is sufficient for the extension's threat model.
- Planned monetization: Free vs Premium tiers via LemonSqueezy license keys (activation codes). Free: 50 follows/unfollows per run, 200 likes/unlikes per day. Premium uncapped. Both one-time lifetime and recurring subscription payment models.
